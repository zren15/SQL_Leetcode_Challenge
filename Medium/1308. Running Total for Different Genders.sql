...
select gender, day, sum(score_points) over (partition by gender order by day) as total #order by day应该在partition by 里面
from Scores


# 错误code
# select gender, day, sum(score_points) over (partition by gender) as total
# from Scores
# order by gender, day